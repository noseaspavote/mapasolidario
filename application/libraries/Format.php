...
class Format {
    ...
    /** CUSTOM FUNTION by @agvago 
     * Encode data as geojson, for points only
     *
     * @param mixed|NULL $data Optional data to pass, so as to override the data passed
     * to the constructor
     * @return string GeoJson representation of a point
     */
    public function to_geojson($data = NULL)
    {
        // If no data is passed as a parameter, then use the data passed
        // via the constructor
        if ($data === NULL && func_num_args() === 0)
        ...
        if (empty($callback) === TRUE)
        {
            ...
            foreach ($data as $row) {
                ...
                if(!isset($properties->latitude)){
                    return json_encode('Latitude not exist', JSON_NUMERIC_CHECK);
                }
                
                if(!isset($properties->longitude)){
                    return json_encode('Longitude not exist', JSON_NUMERIC_CHECK);
                }
